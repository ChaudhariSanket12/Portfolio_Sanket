...
function BikeIntro({ onDone }) {
  ...
  useEffect(() => {
    ...
    function drawExhaustPuffs(s) {
      ...
      s.exhaustPuffs.forEach(p => {
        ...
        ctx.fill();
        p.x -= 2.5; p.y -= 0.6; p.r += 0.7; p.life--;
      });
    }
    ...
    function drawSparks(s) {
      ...
      s.sparks.forEach(sp => {
        ...
        ctx.fill();
        sp.x += sp.vx; sp.y += sp.vy; sp.vy += 0.15; sp.life--; sp.r *= 0.92;
      });
    }
    ...
    function drawBike(bx, wheelAngle, tiltDeg) {
      ...
      const drawWheel = (wx, wy, wr) => {
        ...
        ctx.strokeStyle = "#2a2a3e"; ctx.lineWidth = 9; ctx.stroke();
        ctx.beginPath();
      ...
      ctx.beginPath();
      ctx.moveTo(-55, 0); ctx.lineTo(-10, -75); ctx.lineTo(55, -60);
      ctx.lineTo(35, -30); ctx.lineTo(10, 0); ctx.closePath();
      ctx.fillStyle = "#13131f"; ctx.fill();
      ...
      ctx.beginPath();
      ctx.moveTo(-20, -18); ctx.quadraticCurveTo(-48, -10, -60, 0);
      ctx.strokeStyle = "#777"; ctx.lineWidth = 6; ctx.lineCap = "round"; ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(35, -30); ctx.lineTo(55, 0);
      ctx.strokeStyle = "#666"; ctx.lineWidth = 6; ctx.lineCap = "round"; ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(25, -72); ctx.lineTo(48, -68); ctx.lineTo(48, -55);
      ctx.strokeStyle = "#777"; ctx.lineWidth = 5; ctx.lineCap = "round"; ctx.lineJoin = "round"; ctx.stroke();
      ctx.beginPath();
      ...
      ctx.lineTo(CW - bx + 60, -40 - (CW - bx) * 0.05);
      ctx.closePath();
      ctx.fillStyle = "rgba(251,191,36,0.04)"; ctx.fill();
      ctx.save();
      ctx.translate(-2, -78);
      ctx.beginPath();
      ...
      ctx.strokeStyle = "#fbbf24"; ctx.lineWidth = 3; ctx.lineCap = "round"; ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(-4, -26); ctx.lineTo(10, -26);
      ctx.strokeStyle = "#f97316"; ctx.lineWidth = 2.5; ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(10, -28); ctx.quadraticCurveTo(26, -24, 36, -68);
      ctx.strokeStyle = "#1a1a2e"; ctx.lineWidth = 7; ctx.lineCap = "round"; ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(2, 0); ctx.lineTo(6, 20); ctx.lineTo(18, 18);
      ctx.strokeStyle = "#111"; ctx.lineWidth = 6; ctx.lineCap = "round"; ctx.lineJoin = "round"; ctx.stroke();
      ctx.restore();
      ctx.restore();
    }
    ...
    function tick() {
      ...
      drawSky(); drawStars(s); drawMoon(); drawGround();
      let speed = 0, tilt = 0;

      if (s.phase === "ride-in") {
        speed = 9; s.bikeX += speed; s.wheelAngle += speed / 36;
        drawRoadDashes(s, speed);
        if (s.frame % 3 === 0) spawnPuff(s, s.bikeX);
        if (s.bikeX >= centerX - 10) { s.phase = "brake"; s.skidX = s.bikeX - 20; }
        ...
        if (speed < 0.5) { s.phase = "reveal"; s.shakeX = 0; s.shakeY = 0; }
      } else if (s.phase === "reveal") {
        drawRoadDashes(s, 0);
        s.textAlpha = Math.min(1, s.textAlpha + 0.025);
        s.textScale = Math.min(1, s.textScale + 0.022);
        if (s.textAlpha >= 1 && s.textScale >= 1) { s.phase = "hold"; s.holdTimer = 0; }
        ...
        if (s.opacity <= 0) { s.phase = "done"; onDone(); return; }
      }

      drawSkidMark(s); drawExhaustPuffs(s); drawSparks(s);
      drawBike(s.bikeX, s.wheelAngle, tilt);
      if (s.phase === "reveal" || s.phase === "hold" || s.phase === "fade-out") drawNameReveal(s);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 9999, background: "#0a0a0e", display: "flex", alignItems: "center", justifyContent: "center" }}>
...
const PROJECTS = [
  ...
  },
  {
    ...
    year: "2025–26", emoji: "🎯", color: "#8b5cf6",
  },
  {
    name: "RentEz",
  ...
  Resume
      ...
      {menuOpen && (
        <div className="sc-mobile-menu">
          {NAV_LINKS.map((l) => (
            <button key={l} className="sc-mobile-btn" onClick={() => scrollTo(l.toLowerCase())}>{l}</button>
          ))}
          <a href="/public/Resume - Sanket Chaudhari.pdf" target="_blank" rel="noopener noreferrer" className="sc-mobile-resume">Resume ↗</a>
        </div>
      )}
      ...
      <section id="hero" ref={heroRef} className="sc-hero">
        <div className="sc-road">
          {[...Array(8)].map((_, i) => (
            <div key={i} className="sc-road-line" style={{ animationDelay: `${i * 0.15}s` }} />
          ))}
        </div>
        {[...Array(20)].map((_, i) => (
          <div key={i} className="sc-particle" style={{
            left: `${5 + (i * 4.7) % 90}%`,
            top: `${10 + (i * 7.3) % 80}%`,
            width: i % 3 === 0 ? "3px" : "2px",
        ...
        <div className="sc-hero-content">
          <div className="sc-gauge-wrap">
            <svg width="160" height="95" viewBox="0 0 180 110" className="sc-gauge-svg">
              <defs>
              ...
              {[0, 25, 50, 75, 100].map((v) => {
                ...
                const r1 = 58, r2 = 68;
                return <line key={v} x1={90 + r1 * Math.cos(a)} y1={95 + r1 * Math.sin(a)} x2={90 + r2 * Math.cos(a)} y2={95 + r2 * Math.sin(a)} stroke="#444" strokeWidth="2" />;
              })}
          ...
          <p className="sc-hero-tagline">
            ...
            <span className="sc-accent">Production-grade systems</span> by day,<br />
            open roads by soul.
          </p>
          <div className="sc-hero-btns">
            <button className="sc-primary-btn" onClick={() => scrollTo("projects")}>
              View Projects <span className="sc-btn-arrow">→</span>
            </button>
            <a href="mailto:chaudharisanket2003@gmail.com" className="sc-ghost-btn">Let's Talk</a>
          </div>
          <div className="sc-hero-links">
            <a href="https://linkedin.com/in/sanketkchaudhari" className="sc-social-link" target="_blank" rel="noreferrer">LinkedIn</a>
            <span className="sc-social-divider">·</span>
            <a href="https://github.com/ChaudhariSanket12" className="sc-social-link" target="_blank" rel="noreferrer">GitHub</a>
            <span className="sc-social-divider">·</span>
            <a href="https://sanketchaudhari.tech" className="sc-social-link" target="_blank" rel="noreferrer">Website</a>
          </div>
        </div>
      ...
      <section id="skills" className="sc-section sc-section--dark">
        ...
        <h2 className="sc-section-title">The <span className="sc-accent">tech stack</span> in the garage.</h2>
          ...
          {Object.entries(SKILLS).map(([cat, items]) => (
            <div key={cat} className="sc-skill-group">
              ...
              <div className="sc-skill-tags">
                {items.map((s) => <span key={s} className="sc-skill-tag">{s}</span>)}
              </div>
            </div>
          ))}
        </div>
      ...
      <section id="experience" className="sc-section">
        ...
        <h2 className="sc-section-title">Miles <span className="sc-accent">on the road.</span></h2>
            ...
            <div key={i} className="sc-exp-card">
              ...
              <div className="sc-exp-right">
                <div className="sc-exp-header">
                  <div>
                    ...
                    <div className="sc-exp-company">
                      {e.link ? <a href={e.link} className="sc-exp-link" target="_blank" rel="noreferrer">{e.company} ↗</a> : e.company}
                    </div>
                  </div>
                ...
                <ul className="sc-exp-points">
                  {e.points.map((p, pi) => <li key={pi} className="sc-exp-point">{p}</li>)}
                </ul>
              </div>
            </div>
          ))}
      ...
      <section id="projects" className="sc-section sc-section--dark">
        ...
        <h2 className="sc-section-title">Builds that <span className="sc-accent">actually shipped.</span></h2>
      ...
      <section id="education" className="sc-section">
        ...
        <h2 className="sc-section-title">The <span className="sc-accent">pit stop</span> years.</h2>
        ...
        <h2 className="sc-section-title">Badges on the <span className="sc-accent">jacket.</span></h2>
        <div className="sc-cert-grid">
          {[
            { title: "Java Spring Framework, Spring Boot & Spring AI", issuer: "Udemy – Navin Reddy, Telusko", meta: "Dec 2025 · 55 hrs", icon: "🏅", color: "#f97316" },
            { title: "Communication Skills for BPM Industry", issuer: "Wadhwani Foundation", meta: "Professional Certification", icon: "🎤", color: "#8b5cf6" },
            { title: "Event Host & Anchor", issuer: "Annual College Fest 2025", meta: "JSPM RSCOE, Pune", icon: "🎙️", color: "#06b6d4" },
          ].map((cert) => (
            <div key={cert.title} className="sc-cert-card" style={{ borderColor: cert.color + "33" }}>
              <div className="sc-cert-icon">{cert.icon}</div>
              <div>
                <div className="sc-cert-title">{cert.title}</div>
                <div className="sc-cert-issuer" style={{ color: cert.color }}>{cert.issuer}</div>
...
function SectionLabel({ label }) {
  return (
    <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "11px", color: "#f97316", letterSpacing: "3px", marginBottom: "14px" }}>
      {label}
    </div>
  );
}
...
function InfoRow({ icon, label, val }) {
  return (
    <div style={{ display: "flex", gap: "10px", padding: "11px 0", borderBottom: "1px solid #1a1a2e", alignItems: "flex-start" }}>
      <span style={{ flexShrink: 0 }}>{icon}</span>
      <span style={{ color: "#666", fontSize: "12px", minWidth: "72px", fontFamily: "JetBrains Mono, monospace", flexShrink: 0 }}>{label}</span>
      <span style={{ color: "#ccc", fontSize: "12px", wordBreak: "break-word" }}>{val}</span>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div className="sc-proj-top">
        <div className="sc-proj-emoji" style={{ color: project.color }}>{project.emoji}</div>
        <span className="sc-proj-year" style={{ color: project.color }}>{project.year}</span>
      </div>
      <h3 className="sc-proj-name">{project.name}</h3>
      <div className="sc-proj-tagline" style={{ color: project.color }}>{project.tagline}</div>
      <p className="sc-proj-desc">{project.desc}</p>
      <div className="sc-proj-tags">
        {project.tags.map((t) => (
          <span key={t} className="sc-tech-tag" style={{ borderColor: project.color + "44", color: project.color }}>{t}</span>
        ))}
      </div>
      <div className="sc-proj-features">
        {project.features.map((f) => <span key={f} className="sc-feature-chip">✓ {f}</span>)}
      </div>
      {project.link && project.link !== "#" && (
        <a href={project.link} target="_blank" rel="noreferrer" className="sc-proj-link" style={{ color: project.color }}>
          View on GitHub →
        </a>
      )}
    </div>
